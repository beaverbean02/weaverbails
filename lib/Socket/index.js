...
const { attachWeaverFeatures, patchConnectionConfig } = require('../weaver-features')
...
const makeWASocket = config => {
	...
	const featureConfig = patchConnectionConfig(newConfig)
	featureConfig.logger?.debug?.(
		{ initialFullSyncDone, effectiveSyncFullHistory, userExplicitSyncFlag },
		'computed syncFullHistory policy'
	)
	const baseSock = (0, communities_1.makeCommunitiesSocket)(featureConfig)
	...
	return attachTextRouter(attachWeaverFeatures(finalSock))
}
